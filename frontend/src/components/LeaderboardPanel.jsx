...
export default function LeaderboardPanel({ players = [], leaderboard = [] }) {
  ...
  return (
    <div className="frost-card rounded-[20px] p-6">
      <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t('game.leaderboard')}</h3>

      {!sorted.length ? (
        <p className="mt-4 text-sm text-slate-500 dark:text-slate-300">{t('game.noScores')}</p>
      ) : (
        <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700">
          <div className="grid grid-cols-[56px,1fr,96px] bg-slate-50 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:bg-slate-800 dark:text-slate-300">
            ...
            <span className="text-right">{t('common.score')}</span>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {topRows.map((row, index) => (
              <div key={keyOf(row)} className="grid grid-cols-[56px,1fr,96px] px-4 py-3 text-sm">
                ...
                <span className="font-semibold text-slate-900 dark:text-slate-100">{row.name === 'Guest' ? t('common.guest') : row.name}</span>
      ...
      {remaining > 0 && <p className="mt-3 text-xs font-semibold text-slate-500 dark:text-slate-300">{t('game.remainingPlayers', { count: remaining })}</p>}
    </div>
  )
}
